...
contract ShortsTrackerTimelock {
    ...
    function signalSetGov(
        ...
        bytes32 action = keccak256(
            abi.encodePacked("setGov", _shortsTracker, _gov)
        );
        _setPendingAction(action);
    ...
    function signalSetAveragePriceUpdateDelay(
        uint256 _averagePriceUpdateDelay
    ) external onlyAdmin {
        bytes32 action = keccak256(
            abi.encodePacked(
    ...
    function setAveragePriceUpdateDelay(
        uint256 _averagePriceUpdateDelay
    ) external onlyAdmin {
        bytes32 action = keccak256(
            abi.encodePacked(
    ...
    function signalSetMaxAveragePriceChange(
        uint256 _maxAveragePriceChange
    ) external onlyAdmin {
        bytes32 action = keccak256(
    ...
    function setMaxAveragePriceChange(
        uint256 _maxAveragePriceChange
    ) external onlyAdmin {
        bytes32 action = keccak256(
            abi.encodePacked("setMaxAveragePriceChange", _maxAveragePriceChange)
    ...
    function disableIsGlobalShortDataReady(
        IShortsTracker _shortsTracker
    ) external onlyAdmin {
    ...
    function setGlobalShortAveragePrices(
        ...
        for (uint256 i = 0; i < _tokens.length; i++) {
            ...
            uint256 diff = newAveragePrice > oldAveragePrice
                ? newAveragePrice.sub(oldAveragePrice)
                : oldAveragePrice.sub(newAveragePrice);
            require(
                diff.mul(BASIS_POINTS_DIVISOR).div(oldAveragePrice) <
            ...
            require(
                block.timestamp >=
                    lastUpdated[token].add(averagePriceUpdateDelay),
